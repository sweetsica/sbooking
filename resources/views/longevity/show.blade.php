...
<button type="button"
        onclick="openApprove({{ $booking->id }}, @js($booking->khachHang?->ho_ten ?? 'khách'), @js($booking->co_so_id), @js($booking->gio_thuc_hien ? substr($booking->gio_thuc_hien,0,5) : ''), @js($booking->gio_ket_thuc ? substr($booking->gio_ket_thuc,0,5) : ''), @js($booking->tiep_don_user_id), @js($booking->ghi_chu ?? ''), {source_group: @js($booking->nguon ?? ''), creator_id: @js($booking->nguoi_tao_id), creator_name: @js($booking->nguoiTao?->name ?? ''), tele_owner_name: @js($booking->tele_owner_name ?? ''), ngay_dat: @js(optional($booking->ngay_dat)->toDateString())})"
