...
<!-- Brand Identity -->
<a href="/{{ $coSo->slug }}/lich-hen" class="flex items-center gap-2 shrink-0 mr-2">
...
<h1 class="text-headline-md font-bold text-on-surface leading-tight whitespace-nowrap">Longevity Booking</h1>
</a>
<!-- Navigation Links -->
<nav class="flex items-center gap-1 shrink-0">
@foreach ($items as $it)
@php $on = $active === $it['key']; @endphp
<a href="{{ $it['href'] }}" class="px-4 py-2 text-body-md rounded-lg flex items-center gap-2 whitespace-nowrap transition-colors {{ $on ? 'font-bold bg-secondary-container text-on-secondary-container' : 'text-on-surface-variant hover:bg-surface-container-low hover:text-on-surface' }}">
...
<form method="GET" action="/{{ $coSo->slug }}/tim-kiem" class="relative w-72 hidden md:block">
...
@endphp
@if ($dsCoSo->count() > 1)
<div class="relative hidden md:block" title="Chuyển cơ sở">
...
<select onchange="if(this.value)window.location.href=this.value" class="appearance-none pl-9 pr-8 py-1.5 bg-surface-container-low border-none rounded-xl text-body-sm font-semibold text-on-surface focus:ring-2 focus:ring-secondary/20 cursor-pointer transition-all">
