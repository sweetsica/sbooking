...
<title>Đặt Lịch Tư Vấn - Precision Wellness</title>
...
<a href="/{{ $coSo->slug }}/lich-tu-van" class="p-2 hover:bg-surface-container-low rounded-full transition-all">
<span class="material-symbols-outlined">arrow_back</span>
</a>
<h2 class="text-headline-md font-headline-md font-extrabold text-on-surface">Đặt Lịch Tư Vấn</h2>
...
<h3 class="text-headline-lg font-headline-lg text-on-primary">Đăng Ký Tư Vấn</h3>
...
<form class="p-8" id="lichhen-form" method="POST" action="/{{ $coSo->slug }}/dat-kham">
@csrf
...
<input id="ngay_hen" name="ngay_hen" value="{{ old('ngay_hen', now()->toDateString()) }}" required class="w-full px-4 py-2.5 bg-surface border border-outline rounded-lg form-input-focus transition-all text-body-md" type="date"/>
</div>
<div>
...
<option value="{{ $bs->id }}" @selected(old('bac_si_tu_van_id')==$bs->id)>{{ $bs->ten_day_du }} ({{ $bs->thoi_gian_kham }}p/ca)</option>
@endforeach
</select>
</div>
<div>
<label class="block text-body-sm font-semibold text-on-surface-variant mb-1.5">Ca khám <span class="text-error">*</span></label>
<select id="ca_kham" name="ca_kham_id" required data-old="{{ old('ca_kham_id') }}" class="w-full px-4 py-2.5 bg-surface border border-outline rounded-lg form-input-focus transition-all text-body-md font-time-slot"></select>
</div>
</div>
</div>
...
<div>
<textarea name="ghi_chu" rows="3" class="w-full px-4 py-2.5 bg-surface border border-outline rounded-lg form-input-focus transition-all text-body-md" placeholder="Ghi chú thêm (tuỳ chọn)...">{{ old('ghi_chu') }}</textarea>
</div>
</div>
</div>
...
(function () {
    ...
    async function loadCaKham() {
        ...
        if (bacSi && bacSi.value) {
            try {
                const r = await fetch(`/${slug}/dat-kham/ca-kham?bac_si_id=${bacSi.value}&ngay=${encodeURIComponent(ngay ? ngay.value : '')}`);
                data = await r.json();
            } catch (e) {}
        }
